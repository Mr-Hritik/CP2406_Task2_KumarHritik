...
public class Car {
    ...
    private boolean canMoveForward(){
        ...
        if(xPosition+width >= getRoadCarIsOn().getRoadLength()*25-25+getRoadCarIsOn().getRoadXPosition() && (direction.equals("east") || direction.equals("south"))
            ...
            if (getRoadCarIsOn().getTrafficLight() == null) {
                ...
                return light.getRecentColor().equals("Green");
            }
        }
        return true;
    }
    private int getIndexOfRecentRoad(){
    ...
    private Road nextRoad(){
        ...
        int recentRoadXPosition;
        int recentRoadYPosition;
        int recentRoadEndXPosition;
        int recentRoadEndYPosition;
        Road recentRoad = Map.roads.get(getIndexOfRecentRoad());
        ...
        if(recentRoad.getOrientation().equals("Vertical")){
            recentRoadXPosition = recentRoad.getRoadYPosition();
            recentRoadYPosition = recentRoad.getRoadXPosition();
            recentRoadEndXPosition = recentRoad.getEndRoadYPosition();
            recentRoadEndYPosition = recentRoad.getEndRoadXPosition();
        }
        else{
            recentRoadXPosition = recentRoad.getRoadXPosition();
            recentRoadYPosition = recentRoad.getRoadYPosition();
            recentRoadEndXPosition = recentRoad.getEndRoadXPosition();
            recentRoadEndYPosition = recentRoad.getEndRoadYPosition();
        }
        for (int i = 0; i < Map.roads.size(); i++) {
            ...
            if(r != recentRoad) {
                ...
                if(recentRoad.getTrafficDirection().equals("east") && otherRoadXPosition > recentRoadEndXPosition) {
                    xPositions.add(otherRoadXPosition);
                }
                else if(recentRoad.getTrafficDirection().equals("west") && otherRoadEndXPosition < recentRoadXPosition ){
                    xPositions.add(otherRoadEndXPosition);
                }
                else if(recentRoad.getTrafficDirection().equals("north") && otherRoadEndYPosition < recentRoadYPosition) {
                    yPositions.add(otherRoadEndYPosition);
                }
                else if(recentRoad.getTrafficDirection().equals("south") && otherRoadYPosition > recentRoadEndYPosition){
        ...
        if(recentRoad.getTrafficDirection().equals("east") || recentRoad.getTrafficDirection().equals("west")) {
        ...
        else if(recentRoad.getTrafficDirection().equals("south") || recentRoad.getTrafficDirection().equals("north")) {
            for (int j = 0; j < xPositions.size(); j++) { // loops through every position
